  ...
  return (
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_18px_50px_-34px_rgba(2,21,62,0.4)]">
      ...
      <header className="flex flex-col gap-5 px-5 py-6 sm:px-7 md:flex-row md:items-center md:justify-between lg:px-8">
        <div className="flex items-start gap-4">
          <div className="grid size-11 shrink-0 place-items-center rounded-xl bg-emerald-50 text-emerald-800 ring-1 ring-emerald-100">
            <UsersRound size={21} strokeWidth={1.8} aria-hidden="true" />
          </div>
          <div>
            <p className="text-xs font-bold tracking-[0.15em] text-emerald-800 uppercase">People management</p>
            <h1 className="mt-1 text-2xl font-semibold tracking-[-0.025em] text-[#073F35] sm:text-[28px]">Team Members</h1>
            <p className="mt-1.5 text-sm text-slate-500">Manage your team and their roles.</p>
          </div>
        </div>
        <button type="button" onClick={() => openForm()} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#073F35] px-5 text-sm font-semibold text-white shadow-[0_12px_28px_-15px_rgba(7,63,53,0.9)] transition hover:-translate-y-0.5 hover:bg-emerald-950 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-700 sm:w-auto">
      ...
      {isFormOpen && (
        <div className="border-y border-slate-200 bg-slate-50/70 px-5 py-6 sm:px-7 lg:px-8">
          <div className="mb-5 flex items-start justify-between gap-4">
            <div>
              <h2 className="text-lg font-semibold text-[#073F35]">{editingMember ? 'Edit team member' : 'Add a team member'}</h2>
              <p className="mt-1 text-sm text-slate-500">Enter the member’s professional details.</p>
            </div>
          ...
          <form onSubmit={saveMember} className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="flex flex-col-reverse gap-3 sm:col-span-2 sm:flex-row sm:justify-end xl:col-span-4">
              ...
              <button type="submit" className="min-h-11 rounded-xl bg-[#073F35] px-5 text-sm font-semibold text-white hover:bg-emerald-950">{editingMember ? 'Save changes' : 'Add member'}</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="px-4 py-5 sm:px-6 lg:px-8">
        <div className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-50/60 p-3 md:grid-cols-[minmax(220px,1fr)_minmax(150px,0.48fr)_minmax(150px,0.48fr)_auto]">
          ...
          <div className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-[#073F35]"><Filter size={16} aria-hidden="true" /> Filter</div>
        </div>
      </div>
    </section>
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_18px_50px_-34px_rgba(2,21,62,0.4)]">
      <div className="border-b border-slate-200 px-5 py-5 sm:px-8">
        <h2 className="text-lg font-semibold text-[#073F35]">Team directory</h2>
