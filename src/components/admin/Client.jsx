  ...
  return (
    <div className="space-y-7">
      <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_18px_50px_-34px_rgba(2,21,62,0.4)]">
        <div className="h-1 bg-gradient-to-r from-[#02153E] via-emerald-700 to-teal-500" />
        <header className="flex flex-col gap-5 px-5 py-6 sm:px-7 md:flex-row md:items-center md:justify-between lg:px-8">
          <div className="flex items-start gap-4">
            <div className="grid size-11 shrink-0 place-items-center rounded-xl bg-emerald-50 text-emerald-800 ring-1 ring-emerald-100"><UserRound size={21} aria-hidden="true" /></div>
            <div><p className="text-xs font-bold tracking-[0.15em] text-emerald-800 uppercase">Client management</p><h1 className="mt-1 text-2xl font-semibold tracking-[-0.025em] text-[#073F35] sm:text-[28px]">Clients</h1><p className="mt-1.5 text-sm text-slate-500">Manage your clients and their information.</p></div>
          </div>
          <button type="button" onClick={() => openForm()} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#073F35] px-5 text-sm font-semibold text-white shadow-[0_12px_28px_-15px_rgba(7,63,53,0.9)] transition hover:-translate-y-0.5 hover:bg-emerald-950 sm:w-auto"><Plus size={17} /> Add New Client</button>
        </header>

        {isFormOpen && <div className="border-y border-slate-200 bg-slate-50/70 px-5 py-6 sm:px-7 lg:px-8">
          <div className="mb-5 flex items-start justify-between gap-4"><div><h2 className="text-lg font-semibold text-[#073F35]">{editingClient ? 'Edit client' : 'Add a new client'}</h2><p className="mt-1 text-sm text-slate-500">Enter the client’s contact and account details.</p></div><button type="button" onClick={closeForm} aria-label="Close form" className="grid size-9 shrink-0 place-items-center rounded-lg border border-slate-200 bg-white text-slate-500"><X size={17} /></button></div>
        ...
        <div className="px-4 py-5 sm:px-6 lg:px-8"><div className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-50/60 p-3 md:grid-cols-[minmax(230px,1fr)_minmax(150px,0.45fr)_minmax(160px,0.45fr)_auto]">
          ...
          <div className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-[#073F35]"><Filter size={16} /> Filter</div>
        </div></div>
      </section>

      <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_18px_50px_-34px_rgba(2,21,62,0.4)]">
        <div className="border-b border-slate-200 px-5 py-5 sm:px-8"><h2 className="text-lg font-semibold text-[#073F35]">Client directory</h2><p className="mt-1 text-sm text-slate-500">View and manage all client records.</p></div>
...
function Avatar({ name }) { return <span className="grid size-11 shrink-0 place-items-center rounded-full bg-emerald-50 text-xs font-bold text-emerald-800 ring-4 ring-slate-50">{name.split(' ').map((part) => part[0]).join('').slice(0, 2)}</span> }
function Status({ value }) { return <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${value === 'Active' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{value}</span> }
function Actions({ client, onEdit, onDelete }) { return <div className="flex justify-end gap-2"><button type="button" onClick={() => onEdit(client)} aria-label={`Edit ${client.name}`} className="grid size-10 place-items-center rounded-xl border border-slate-200 text-[#123B4A] hover:bg-emerald-50"><Pencil size={15} /></button><button type="button" onClick={() => onDelete(client.id)} aria-label={`Delete ${client.name}`} className="grid size-10 place-items-center rounded-xl border border-red-100 text-red-600 hover:bg-red-50"><Trash2 size={15} /></button></div> }
